...
function isRealDate(dateStr: string): boolean {
  ...
  return (
    date.getUTCFullYear() === parseInt(parts[0], 10) &&
...
export const validateEventId: ValidatorFn = (req, res, next) => {
  const { eventId } = req.params;
  if (!eventId || !UUID_REGEX.test(eventId)) {
    sendError(res, 'Invalid event ID', 400);
    return;
  }
  next();
};
...
// ── Create event ──
export const validateCreateEvent: ValidatorFn = (req, res, next) => {
  const {
    title,
    person_name,
    event_type,
...
// ── Update event ──
export const validateUpdateEvent: ValidatorFn = (req, res, next) => {
  const {
    title,
    person_name,
    event_type,
...
// ── RSVP respond ──
export const validateRSVP: ValidatorFn = (req, res, next) => {
...
export const validateEventQuery: ValidatorFn = (req, res, next) => {
  ...
  if (type !== undefined) {
    if (!VALID_EVENT_TYPES.includes(type as string)) {
      errors.push(`type filter must be one of: ${VALID_EVENT_TYPES.join(', ')}`);
    }
  }

  if (rec !== undefined) {
    if (!VALID_RECURRENCE.includes(rec as string)) {
      errors.push(`recurrence filter must be one of: ${VALID_RECURRENCE.join(', ')}`);
    }
  }

  if (req.query.search && (req.query.search as string).length > 100) {
