...
const execution_guard_1 = require("./execution-guard");
const scheduler_logger_1 = require("./scheduler.logger");
const reminder_service_1 = require("../jobs/reminder.service");
const logger = new scheduler_logger_1.SchedulerLogger('CronManager');
class CronManager {
    constructor(config) {
        this.config = config;
        this.tasks = new Map();
        ...
        this.guard = new execution_guard_1.ExecutionGuard(config.instanceId, config.lockTtlSeconds);
    }
    /**
    ...
    start() {
        ...
        if (!node_cron_1.default.validate(this.config.reminderCron)) {
        ...
        this.registerJob('daily-reminders', this.config.reminderCron, () => this.executeDailyReminders());
        // Job 2: Weekly lock cleanup (every Sunday at 3 AM)
        this.registerJob('lock-cleanup', '0 3 * * 0', () => this.executeLockCleanup());
     ...
     */
    registerJob(name, expression, handler) {
        const task = node_cron_1.default.schedule(expression, async () => {
    ...
    async executeDailyReminders() {
        ...
        try {
            const result = await this.executeWithTimeout(() => reminder_service_1.reminderService.processReminders(), this.config.executionTimeoutMs);
